...
public:
	RBTreeNode(RBTreeNode *parent, string key, void *data) {
		this->parent_ = parent;
		this->left_ = NULL;
		this->right_ = NULL;
		this->color_ = RED_;
	...
	void setKey(string key) { this->key_ = key; }
	void **getData() { return &this->data_; }
	string getKey() { return this->key_; }
...
private:
	...
	RBTreeNode *treeInsert_(RBTreeNode *tree, string key, void *data) {
		RBTreeNode *inserted;
		if (key < tree->getKey()) {
	...
	RBTreeNode *createNode_(RBTreeNode *parent, short loc, string key, void *data) {
	...
	RBTreeNode *queryTreeRecur_(RBTreeNode *tree, string key) {
		if (tree == NULL) return NULL;
	...
	void printTreeRecur_2D_(RBTreeNode *node, string lpad, string rpad) {
		...
		string ii = (node->getParent() != NULL) ? node->getParent()->getKey() : "null";
...
public:
	RBTree() { root_ = NULL; }
	RBTree(string key, void *data) { root_ = new RBTreeNode(NULL, key, data); }
	~RBTree() { delete this->root_; }
	void setTrace(int flag) { trace = flag; }
	int getTrace() { return trace; }
	void toggleTrace() { trace++; }
	RBTreeNode *insertNode(string key, void *data) {
		if ((getTrace() % 4) > 0) std::cout << "\n ==> A D D I N G    " << key << " <==\n";
		return treeInsert_(this->root_, key, data);
	}
	RBTreeNode *queryTree(string key) { return queryTreeRecur_(this->root_, key); }
...
int main() {
	string choice;
	bool stay = true;
	RBTree *myTree = new RBTree("A1", NULL);
	cout << "\nCreated with " << 60 << endl;
	myTree->setTrace(2);
	cout << "Turn Trace/Step ON \n";

	myTree->insertNode("31", NULL);
	myTree->insertNode("1A", NULL);
	myTree->insertNode("D1", NULL);
	myTree->insertNode("D9", NULL);
	myTree->insertNode("HF", NULL);
	myTree->insertNode("DA", NULL);
	myTree->insertNode("EE", NULL);
	myTree->insertNode("4B", NULL);
	...
	while (stay) {
		...
		if (choice.size() == 1) {
			...
			case 'A':
				...
				cin >> user_input;
				if (user_input.size() > 2) { cout << "		Size of key must be less then 3 !\n"; break; }
